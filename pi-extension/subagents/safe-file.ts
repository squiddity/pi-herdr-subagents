...
function inspectPath(path: string, label: string): Stats {
  try {
    ...
    if (stats.isSymbolicLink()) {
      throw new UnsafeFileError(`${label} must not be a symbolic link`, "symlink");
    }
    if (!stats.isFile()) {
...
/** Verify that a path resolves to a non-symlink regular file. */
export function assertRegularFile(path: string, label = "file"): void {
  const fd = openRegularFile(path, label);
  closeSync(fd);
}

/** Read a non-symlink regular file without exceeding maxBytes. */
